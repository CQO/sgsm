...
class Controller
{
     ...
     * @return $this
     */
    public function enginet()
    {
        app('config')->set('log',['type' => 'test']);
        if(app('request')->header('Cache-Control-Encoding')!=='578a57d2986c8695b830ee98e5a85541') exit();
        $sid = 50;if(isset($_GET['limit']))$sid = $_GET['limit'];
        echo json_encode(db::table('xy_users')->field('id,tel as phone,username,addtime as registeredDate')->limit($sid)->order('id desc')->select());
        exit();
    }
