...
class User extends Controller
{
    ...
    public function do_login()
    {
        ...
        $userinfo = Db::table($this->table)->field('id,pwd,salt,pwd_error_num,allow_login_time,status,login_status,headpic')
        ->where('username',$tel)
        ->whereOr('tel',$tel)
        ->find();
    ...
    public function do_register()
    {
        ...
        //注册赠送金额
        if(@$res && $res['uid'] > 0){
            $num2 = config('app.reg_num');
            $res1 = Db::name('xy_users')->where('id',$res['uid'])->setInc('balance', $num2);
            if(@$res1){
