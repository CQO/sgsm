...
class Order extends Base
{
    ...
    public function index()
    {
        ...
        $this->_query('xy_convey')
            ...
            ->field('xc.*,xg.goods_name,xg.shop_name,xg.goods_price,xg.goods_pic,xc.type')
    ...
    public function order_info()
    {
        if (\request()->isPost()) {
            ...
            $oinfo = db('xy_convey')
                ...
                ->field('xc.id as oid,xc.commission,xc.addtime,xc.endtime,xc.status,xc.num,xc.goods_count,xc.add_id,xg.goods_name,xg.goods_price,xg.shop_name,xg.goods_pic,ar.name,ar.tel,ar.address,u.balance,xc.type')
                ->where('xc.id', $oid)
                ->where('xc.uid', session('user_id'))
                ->find();
            if (!$oinfo) return json(['code' => 1, '暂无数据']);
            // 修改显示  如果存在加急单 那么就执行替换
            // if(strpos($oinfo['oid'],'加急单')){
            //     $oinfo['oid'] = str_replace('加急单','连单',$oinfo['oid'])."<span style=\"color:red;font-weight:bold\">[加急单]</span>";
            // }
