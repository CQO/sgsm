...
class RotOrder extends Base
{
    ...
    public function index()
    {
        ...
        $res = Db::name('xy_users')->where('id',session('user_id'))->field('commission,team_commission,income,odd_number')->find();
       
            $this->day_deal = Db::name('xy_convey')->where($where)->where('status','in',[1,3,5])->sum('commission');
            $this->day_deal = $this->day_deal +$res['commission'];
//        $this->day_l_count = Db::name('xy_convey')->where($where)->where('status',5)->count('num');//交易冻结单数
        ...
        $addtimess = strtotime(date('Y-m-d'));
        $this->day_d_count = db('xy_convey')->where("uid=".session('user_id')." AND status IN(0,1,3,5) AND (addtime >= $addtimess OR baodantime >= $addtimess)")->count('id');
          $this->day_d_count =  $this->day_d_count + $res['odd_number'];
        ...
        $this->yes_team_num = Db::name('xy_reward_log')->where('uid',session('user_id'))->where('addtime','between',[$yes1,$yes2])->where('status',1)->sum('num');//获取下级返佣数额
          $this->yes_team_num =   $this->yes_team_num+$res['team_commission'];
        ...
        $this->yes_user_yongjin = $this->yes_user_yongjin +$res['income'];
  ...
  /**
    ...
    */
    public function submit_order()
    {
        ...
        if(!$add_id) return json(['code'=>1,'info'=>'还没有设置身份信息']);
        //检查交易状态
        ...
        $count = db('xy_goods_list')->where('cid','=',$cid)->count();  //获取库存数量
        if($count < 1) return json(['code'=>1,'info'=>'抢单失败，商品库存不足！']);
        
        $res = model('admin/Convey')->create_order($uid,$cid);  //获取单子
        return json($res);
    }
