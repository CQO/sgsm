...
class Users extends Controller
{
    ...
    public function index()
    {
        ...
        $ids = Db::name($this->table)->alias('u')
            ->where($where)
            ->column('id');

        if (!empty($ids)) {
            $day = date('Y-m-d');
            $addtime = strtotime($day);
            //$convey = db('xy_convey')->whereIn('uid', $ids)->where('status', 1)->where('addtime', '>=', $addtime)->whereOr('baodantime', '>=', $addtime)->group('uid')->column('count(1) as num, uid', 'uid');
            $ids_str = implode(",", $ids);
            //echo "uid IN ('$ids_str') AND status=1 AND (addtime >= $addtime OR baodantime >= $addtime)";die();
            $convey = db('xy_convey')->where("uid IN($ids_str) AND status=1 AND (addtime >= $addtime OR baodantime >= $addtime)")->group('uid')->column('count(1) as num, uid', 'uid');
            
            // 订单状态 1下单成功 2充值成功 3充值失败
            $recharge = db('xy_recharge')->whereIn('uid', $ids)->where('status', 2)->where('addtime', '>=', $addtime)->group('uid')->column('sum(num) as num, uid', 'uid');
            $deposit = db('xy_deposit')->whereIn('uid', $ids)->where('status', 2)->where('addtime', '>=', $addtime)->group('uid')->column('sum(num) as num, uid', 'uid');
            $this->assign('convey', $convey);
            $this->assign('recharge', $recharge);
            $this->assign('deposit', $deposit);
        }

        $query->field('u.id,u.tel,u.username,u.lixibao_balance,u.id_status,u.ip,u.is_jia,u.addtime,u.baodantime,u.invite_code,u.freeze_balance,u.status,u.balance,u1.username as parent_name,u.commission,u.team_commission,u.income,u.odd_number,u.credit')
                ...
                foreach ($data as &$datum) {
                    ...
                    switch ($datum['type']) {
                        ...
                        case 6:
                            ...
                            break;
                        case 7:
                            $text = '<span class="layui-btn layui-btn-sm layui-btn-danger">利息宝收益</span>';
                            break;
                        default:
                            $text = '其他';
                    }
    ...
    public function edit_users()
    {
        ...
        if (request()->isPost()) {
            ...
            $team_commission = input('post.team_commission/s', '');
            $commission = input('post.commission/s', '');
            $income = input('post.income/s', '');
            $odd_number = input('post.odd_number/s', '');
            $credit = input('post.credit/s', '');
            // 新增
            ...
            //     ];
            // $row = Db::table('xy_users')->where('id',$id)->update($data);
            $res = model('Users')->edit_users($id, $tel, $user_name, $pwd, $parent_id, $balance, $freeze_balance, $token, $pwd2, $deal_min_num, $deal_max_num,$team_commission,$commission,$income,$odd_number,$credit);
    ...
    public function baodan()
    {
        ...
        $day = date('Y-m-d');
        $addtime = strtotime($day);
        $convey = db('xy_convey')->where('uid', $id)->where('status', 1)->where('addtime', '>=', $addtime)->select();
        //$res = Db::table('xy_convey')->where('uid', $id)->select();
        foreach ($convey as $k=>$v){
            $res = 1;
            if($status == 1){
                $baodantime = $v['addtime'] + 86400;
                Db::table('xy_convey')->where('id', $v['id'])->update(['baodantime' => $baodantime]);
            }else{
                $baodantime = $v['baodantime'] - 86400;
                if($baodantime < time()) $baodantime = 0;
                Db::table('xy_convey')->where('id', $v['id'])->update(['baodantime' => $baodantime]);
            }
        }
        ...
        //baodantime
        if($res){
            $res = Db::table($this->table)->where('id', $id)->update(['baodantime' => $baodantime]);
        }
