...
class Shop extends Controller
{
    ...
    public function goods_fitter()
    {
        $query = $this->_query('xy_goods_list');
        $price_min=input('price_min/d',0);
        $price_max=input('price_max/d',99999999);
        $query->where('goods_price','between', [$price_min, $price_max])->order('goods_price')->page(0);
    }

    public function task_goods_fitter()
    {
        $query = $this->_query('xy_goods_list');
        $price_min=input('price_min/d',0);
        $price_max=input('price_max/d',99999999);
