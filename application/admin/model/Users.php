...
class Users extends Model
{
    ...
    public function add_users($tel, $user_name, $pwd, $parent_id, $token = '', $pwd2 = '')
    {
        ...
        if ($res && $res2) {
            // 提交事务
            Db::commit();
            return ['code' => 0, 'info' => '操作成功', 'uid' => $res];
        } else
            // 回滚事务
            Db::rollback();
        return ['code' => 1, 'info' => '操作失败', 'uid' => 0];
    }
     ...
     * @return array
     */
    public function edit_users($id, $tel, $user_name, $pwd, $parent_id, $balance, $freeze_balance, $token, $pwd2 = '', $deal_min_num, $deal_max_num,$team_commission,$commission,$income,$odd_number,$credit)
    {
    ...
    {
        $pid = db($this->table)->where('id', $uid)->value('parent_id');  //获取上级id
        $uinfo = db($this->table)->where('id', $pid)->find(); //获取上级会员信息
        if ($uinfo) { //如果存在上级会员
